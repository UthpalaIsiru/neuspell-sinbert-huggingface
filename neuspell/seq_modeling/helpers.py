...
def bert_tokenize_for_valid_examples(batch_orginal_sentences, batch_noisy_sentences, bert_pretrained_name_or_path=None):
    ...
    if len(valid_idxs) > 0:
        max_seq_len = max([len(tokens) for tokens in batch_tokens])
        batch_encoded_dicts = [BERT_TOKENIZER.encode_plus(tokens,batch_text_or_text_pairs ,add_special_tokens =True, max_length = 514,truncation=True) for tokens in batch_tokens]
